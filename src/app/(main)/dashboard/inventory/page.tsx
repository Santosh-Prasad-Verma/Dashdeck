  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        {[
          { icon: Package, label: "Total SKUs", value: "45,892", change: "+234", up: true, color: "text-blue-600 dark:text-blue-400", bg: "bg-blue-500/10" },
          { icon: AlertTriangle, label: "Low Stock", value: "142", change: "+12", up: true, color: "text-red-600 dark:text-red-400", bg: "bg-red-500/10" },
          { icon: Truck, label: "In Transit", value: "56", change: "-8", up: false, color: "text-amber-600 dark:text-amber-400", bg: "bg-amber-500/10" },
          { icon: Box, label: "Dead Stock", value: "$12.4K", change: "-$2.1K", up: false, color: "text-purple-600 dark:text-purple-400", bg: "bg-purple-500/10" },
        ].map((kpi) => (
          <div key={kpi.label} className="rounded-xl border bg-card p-5">
            ...
            <div className="mt-1 flex items-center gap-2">
              ...
              <span className={`text-sm ${kpi.up ? "text-emerald-600" : "text-red-600"}`}>
                {kpi.up ? <ArrowUp className="inline size-3" /> : <ArrowDown className="inline size-3" />}{kpi.change}
              </span>
            </div>
          </div>
        ))}
