...
const guides = [
  { 
    ...
    icon: Book,
    content: {
      intro: "Dashdeck is a high-performance admin dashboard built with Next.js 15, Tailwind CSS v4, and Zustand. Follow these steps to configure your environment:",
      steps: [
        { title: "Install dependencies", desc: "Run `npm install` in the workspace directory to install all package packages." },
        { title: "Run locally", desc: "Start the local development server with `npm run dev`. Access the application at http://localhost:3000." },
        { title: "Configuration", desc: "Environment variables can be configured by copying `.env.example` into a new `.env` file." },
        { title: "Verify compilation", desc: "Run `npm run build` to verify production builds and lint constraints via Biome lint." }
      ]
    }
  },
  { 
    ...
    icon: FileText,
    content: {
      intro: "Dashdeck comes preloaded with numerous production-grade tracking screens. Here is a breakdown of the primary layouts:",
      details: [
        { name: "AI & LLM Analytics", desc: "Monitor token consumptions, agent performance metrics, model output accuracies, and cost breakdowns in real-time." },
        { name: "DevOps Monitor", desc: "Keep track of active server nodes, API Gateway requests/sec, database cluster health indicators, and latency graphs." },
        { name: "Healthcare Center", desc: "Analyze patient admission ratios, bed occupancy stats, clinical schedules, and patient satisfaction ratings." },
        { name: "SaaS Metrics", desc: "Visual MRR growth rates, churn analysis, average revenue per user (ARPU), and active trial conversion charts." }
      ]
    }
  },
  { 
    ...
    icon: Zap,
    content: {
      intro: "Dashdeck utilizes a fully dynamic custom design system. You can switch presets or customize colors at the code level:",
      presets: [
        { name: "Default Theme", desc: "A sleek, clean indigo-colored interface with obsidian dark mode overrides." },
        { name: "Tangerine Theme", desc: "A warm, high-contrast amber/orange visual style perfect for operations dashboards." },
        { name: "Brutalist Theme", desc: "Flat shadows, thick dark borders, and clean monospace typography elements." },
        { name: "Soft Pop Theme", desc: "A playful violet-tinted color scheme with rounded corners and high accessibility contrast." }
      ],
      custom: "To inject custom color codes, navigate to `src/app/globals.css` and modify the HSL values under the respective theme classes."
    }
  },
  { 
    ...
    icon: ExternalLink,
    content: {
      intro: "Dashdeck is driven by Zustand store states that run completely on the client side. This allows high-speed state persistence without database overhead:",
      apis: [
        { store: "users-store.ts", desc: "Handles the directory of mock team members, roles, statuses, and updates." },
        { store: "kanban-store.ts", desc: "Orchestrates columns, drag-and-drop state modifiers, task priorities, and task assignments." },
        { store: "calendar-store.ts", desc: "Manages event creation, calendar grid state, filters, and color selections." },
        { store: "chat-store.ts", desc: "Maintains active threads, contacts lists, messaging logs, and status updates." }
      ]
    }
  },
];
                  ...
                  <DialogContent className="max-w-xl bg-card border border-border/50 backdrop-blur-lg rounded-2xl p-6">
                    ...
                    <div className="my-2 space-y-4 text-sm max-h-[60vh] overflow-y-auto pr-2">
                      {guide.id === "getting-started" && guide.content.steps && (
                        <div className="space-y-3">
                          <p className="text-zinc-650 dark:text-zinc-350 text-xs leading-relaxed">{guide.content.intro}</p>
                          <div className="space-y-3">
                            {guide.content.steps.map((step, idx) => (
                              <div key={idx} className="flex gap-3 items-start p-3 rounded-xl border border-border/20 bg-muted/20">
                                ...
                                <div>
                                  <h4 className="font-semibold text-xs text-zinc-900 dark:text-zinc-100">{step.title}</h4>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {guide.content.presets.map((preset, idx) => (
                              <div key={idx} className="p-2.5 rounded-xl border border-border/20 bg-muted/20 flex flex-col gap-1">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-2">
                            {guide.content.apis.map((api, idx) => (
                              <div key={idx} className="p-2.5 rounded-xl border border-border/20 bg-muted/25 flex flex-col gap-1 font-mono">
                    ...
                    <DialogFooter className="border-t border-border/20 pt-4">
                      <DialogTrigger asChild>
                        <Button variant="outline" className="h-9 text-xs">Close Guide</Button>
                      </DialogTrigger>
                    </DialogFooter>
                  </DialogContent>
                </Dialog>
              ...
              {filteredFaqs.map((faq, i) => (
                ...
                >
                  ...
                  <div className="pt-2 flex justify-end">
                    <Badge variant="outline" className="text-[9px] uppercase tracking-wider px-1.5 py-0 bg-muted/40 border-border/20 text-muted-foreground font-bold font-sans">
                      {faq.category}
                    </Badge>
                  </div>
                </motion.div>
      ...
      <div className="relative overflow-hidden flex flex-col sm:flex-row items-center justify-between gap-4 rounded-2xl border border-indigo-500/20 bg-indigo-500/5 dark:bg-indigo-500/10 p-6 shadow-sm">
        ...
        <div className="flex items-center gap-4 text-center sm:text-left z-10">
          ...
          <div>
            ...
            <p className="text-zinc-500 dark:text-zinc-400 text-xs">If you can't find what you're looking for, feel free to open a ticket or search issues.</p>
          </div>
        </div>
        <Button className="bg-indigo-600 hover:bg-indigo-500 text-white dark:bg-indigo-500 dark:hover:bg-indigo-400 shrink-0 font-medium z-10" asChild>
          <Link href="https://github.com/Santosh-Prasad-Verma/Dashdeck/issues" target="_blank" className="inline-flex items-center gap-1.5">
            Open GitHub Issue
