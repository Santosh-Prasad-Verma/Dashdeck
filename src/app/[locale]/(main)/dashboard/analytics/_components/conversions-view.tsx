...
import { CartesianGrid, Funnel, FunnelChart, LabelList, Line, LineChart, XAxis } from "recharts";
          ...
          <CardContent>
            <ChartContainer config={chartConfig} className="h-64 w-full">
              <LineChart data={conversionData} margin={{ top: 0 }}>
                ...
                <Line dataKey="signups" type="natural" stroke="var(--color-signups)" strokeWidth={2} dot={false} />
                <Line dataKey="purchases" type="natural" stroke="var(--color-purchases)" strokeWidth={2} dot={false} />
              </LineChart>
            </ChartContainer>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <ChartContainer config={funnelConfig} className="h-64 w-full">
              <FunnelChart margin={{ top: 10, right: 30, left: 10, bottom: 10 }}>
                ...
                <Funnel dataKey="value" data={chartFunnelData} isAnimationActive>
                  <LabelList
                    position="right"
                    dataKey="name"
                    fill="currentColor"
                  ...
                  <div className="text-right">
                    <div className="font-medium tabular-nums">
                      {typeof goal.current === "number" && goal.current > 1000
                        ...
                        : goal.current}
                      {goal.goal.includes("Rate") || goal.goal.includes("Retention")
                        ? "%"
                        : goal.goal.includes("Revenue")
                          ? ""
                          : ""}
                    </div>
