      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <Card className="bg-card/50 backdrop-blur-xs relative overflow-hidden">
          <CardContent className="p-5 flex flex-col gap-1">
            ...
            <p className="text-muted-foreground text-[11px] truncate">
              Inflows minus outflows across all accounts
            </p>
          </CardContent>
        </Card>
      </div>
            ...
            {accountGroups.map((group, groupIdx) => (
              <div key={group.categoryName} className="flex flex-col">
                ...
                <div className="flex justify-between items-center bg-muted/20 px-6 py-2.5 text-[11px] font-semibold tracking-wider text-muted-foreground border-b">
                  ...
                  <span className="tabular-nums font-bold text-foreground/80">${group.categoryTotal.toLocaleString()}</span>
                </div>
                      ...
                      <div className="flex items-center gap-4 flex-1 min-w-0 max-w-[280px]">
                        ...
                        <div className="flex flex-col min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge variant="outline" className="text-[9px] font-bold py-0 px-1 border-muted-foreground/30 text-muted-foreground bg-muted/40">
                              {item.type}
                            </Badge>
                          </div>
                          <div className="flex items-center gap-2 mt-0.5 text-xs text-muted-foreground">
                            ...
                            <span>·</span>
                            <span className={
                              item.changeType === "up" ? "text-emerald-500 font-medium" :
                              item.changeType === "down" ? "text-red-500 font-medium" :
                      ...
                      <div className="hidden md:block w-36 h-8 shrink-0">
                        <svg className="w-full h-full" viewBox="0 0 140 40" preserveAspectRatio="none">
                          <path
                            ...
                            fill="none"
                            stroke={
                              item.changeType === "up" ? "rgba(16, 185, 129, 0.7)" :
                              item.changeType === "down" ? "rgba(239, 68, 68, 0.7)" :
        ...
        <Card className="bg-card/30 backdrop-blur-xs flex flex-col justify-between">
          ...
          <CardContent className="p-6">
            <div className="flex flex-col gap-4">
              {[
                { name: "Tesco Express", amount: -42.15, account: "Revolut Premium", time: "Today · 18:12", type: "debit" },
                { name: "Apple Pay top-up", amount: 250.00, account: "Revolut Premium", time: "Yesterday · 09:11", type: "credit" },
                { name: "Standing order - Rent", amount: -1180.00, account: "HSBC Bank", time: "May 16 · 09:00", type: "debit" },
                { name: "Spot buy - BTC", amount: -1500.00, account: "Bitcoin", time: "May 15 · 22:48", type: "debit" },
                { name: "Dividend - VWRL", amount: 86.40, account: "Brokerage Account", time: "May 14 · 16:00", type: "credit" },
              ].map((item, i) => (
                <div key={i} className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-3">
                    <div className={`flex size-8 shrink-0 items-center justify-center rounded-lg bg-neutral-800 text-white font-bold text-xs`}>
                      {item.name[0]}
                    </div>
                    <div>
                  ...
                  <span className={`font-semibold tabular-nums text-sm ${item.amount < 0 ? "text-foreground" : "text-emerald-500"}`}>
          ...
          <CardContent className="p-6 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-3">
              {[
                ...
                { name: "Upwork - Project payout", amount: 1412, date: "May 28", account: "Revolut Premium", logo: "U" },
                { name: "VWRL - Quarterly dividend", amount: 86, date: "Jun 12", account: "Brokerage Account", logo: "V" },
              ].map((item, i) => (
                <div key={i} className="flex items-center justify-between rounded-lg border p-3 hover:bg-muted/20 transition-colors">
                  <div className="flex items-center gap-3">
                  ...
                  <span className="font-bold text-emerald-500 text-sm tabular-nums">+${item.amount.toLocaleString()}</span>
                </div>
              ))}
            </div>
          </CardContent>
        ...
        <Card className="bg-card/30 backdrop-blur-xs flex flex-col justify-between">
          ...
          <CardContent className="p-6 flex flex-col gap-4">
            <div className="grid grid-cols-3 gap-3">
              <button type="button" className="flex flex-col items-center justify-center border rounded-lg p-3 hover:bg-muted/30 transition-all text-center gap-2 cursor-pointer group">
            ...
            <Button className="w-full gap-2 border h-10 bg-transparent hover:bg-muted text-foreground hover:text-foreground shadow-none mt-2" variant="outline">
