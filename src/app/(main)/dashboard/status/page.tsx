...
const defaultServices = [
  { id: "web", name: "Web Dashboard", uptime: "99.99%", status: "operational", latency: "45ms", baseLatency: 45, icon: Globe },
  { id: "api", name: "API Gateway", uptime: "99.95%", status: "operational", latency: "32ms", baseLatency: 32, icon: Server },
  { id: "db", name: "Database Cluster", uptime: "99.97%", status: "operational", latency: "8ms", baseLatency: 8, icon: Database },
  { id: "storage", name: "File Storage", uptime: "99.98%", status: "operational", latency: "28ms", baseLatency: 28, icon: Cloud },
  { id: "auth", name: "Authentication API", uptime: "100.00%", status: "operational", latency: "15ms", baseLatency: 15, icon: ShieldCheck },
  { id: "workers", name: "Background Workers", uptime: "99.90%", status: "operational", latency: "120ms", baseLatency: 120, icon: Cpu },
];
...
export default function Page() {
  ...
  const incidents = simulateOutage 
    ? [
        { id: "sim-1", date: "Today, Just now", title: "Background Workers major outage & API Gateway latency spike", status: "investigating", duration: "Active" },
        ...defaultIncidents
      ]
    : defaultIncidents;
  ...
  return (
    <div className="flex flex-col gap-6 max-w-7xl pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-4 bg-card/40 border border-border/30 px-4 py-2.5 rounded-2xl backdrop-blur-md">
          ...
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="outage-simulator" className="text-xs font-semibold text-zinc-650 dark:text-zinc-350 cursor-pointer flex items-center gap-1">
        ...
        <CardContent className="flex items-center gap-4 p-5 z-10 relative">
          ...
          <div>
            ...
            <p className={cn(
              ...
              simulateOutage ? "text-amber-700/70 dark:text-amber-400/60" : "text-emerald-700/70 dark:text-emerald-400/60"
            )}>
              {simulateOutage 
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {services.map((svc) => (
          <Card key={svc.name} className="border-border/40 bg-card/65 backdrop-blur-md hover:bg-card hover:border-border/80 hover:shadow-md hover:shadow-black/5 dark:hover:shadow-black/20 hover:-translate-y-[1px] transition-all duration-300 rounded-2xl overflow-hidden">
            <CardContent className="flex items-start gap-4 p-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-3">
                  ...
                  {svc.status === "outage" ? (
                    <Badge className="bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/20 font-bold text-[9px] uppercase tracking-wider px-2 py-0.5 rounded-md gap-1 shrink-0" variant="outline">
                      <span className="size-1 rounded-full bg-rose-500 animate-pulse" />
                      Outage
                    </Badge>
                  ) : svc.status === "degraded" ? (
                    <Badge className="bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20 font-bold text-[9px] uppercase tracking-wider px-2 py-0.5 rounded-md gap-1 shrink-0" variant="outline">
                      <span className="size-1 rounded-full bg-amber-500 animate-pulse" />
                      Degraded
                    </Badge>
                  ) : (
                    <Badge className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 font-bold text-[9px] uppercase tracking-wider px-2 py-0.5 rounded-md gap-1 shrink-0" variant="outline">
