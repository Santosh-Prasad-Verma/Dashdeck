          ...
          {["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug"].map((month, i) => (
            <div key={month} className="flex flex-1 items-center gap-2">
              <div className="flex flex-1 flex-col gap-1">
                <div className={`h-2 rounded-full ${i < 6 ? "bg-emerald-500" : i === 6 ? "bg-amber-500" : "bg-muted"}`} />
        ...
        {milestones.map((m, i) => (
          <div key={i} className="flex items-start gap-3 rounded-xl border bg-card p-4">
            <div className={`mt-0.5 size-2 shrink-0 rounded-full ${
              m.status === "done" ? "bg-emerald-500" : m.status === "in-progress" ? "bg-blue-500 animate-pulse" : "bg-muted-foreground/30"
            }`} />
            <div>
