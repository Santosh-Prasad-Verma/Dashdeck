      ...
      <CardContent>
        ...
        <ChartContainer config={chartConfig} className="aspect-auto h-72 w-full">
          <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            {/* Income stacked bar */}
            <Bar dataKey="salary" name="Salary" stackId="income" fill="var(--color-salary)" radius={[0, 0, 0, 0]} maxBarSize={30} />
            <Bar dataKey="freelance" name="Freelance" stackId="income" fill="var(--color-freelance)" radius={[4, 4, 0, 0]} maxBarSize={30} />

            {/* Expenses stacked bar (plotted below 0 automatically because values are negative) */}
            <Bar dataKey="rent" name="Rent" stackId="expense" fill="var(--color-rent)" radius={[0, 0, 0, 0]} maxBarSize={30} />
            <Bar dataKey="food" name="Food" stackId="expense" fill="var(--color-food)" radius={[0, 0, 0, 0]} maxBarSize={30} />
            <Bar dataKey="bills" name="Bills" stackId="expense" fill="var(--color-bills)" radius={[0, 0, 0, 0]} maxBarSize={30} />
            <Bar dataKey="leisure" name="Leisure" stackId="expense" fill="hsl(var(--destructive))" radius={[0, 0, 4, 4]} maxBarSize={30} />
          </BarChart>
        </ChartContainer>
      </CardContent>
    </Card>
