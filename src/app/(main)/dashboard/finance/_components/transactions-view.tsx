            ...
            <Table>
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  ...
                  <TableHead className="text-xs font-semibold tracking-wider text-muted-foreground px-4 py-3">Date</TableHead>
                  <TableHead className="text-xs font-semibold tracking-wider text-muted-foreground px-4 py-3">Description</TableHead>
                  <TableHead className="text-xs font-semibold tracking-wider text-muted-foreground px-4 py-3">Account</TableHead>
                  <TableHead className="text-xs font-semibold tracking-wider text-muted-foreground px-4 py-3">Category</TableHead>
                  <TableHead className="text-xs font-semibold tracking-wider text-muted-foreground px-4 py-3 text-right">Amount</TableHead>
                  <TableHead className="text-xs font-semibold tracking-wider text-muted-foreground px-4 py-3">Status</TableHead>
                ...
                {filteredTransactions.length === 0 ? (
                  ...
                  filteredTransactions.map((tx) => (
                    <TableRow key={tx.id} className="hover:bg-muted/10 group">
                      ...
                      <TableCell className="text-xs text-muted-foreground whitespace-nowrap px-4 py-4">{tx.date}</TableCell>
                      ...
                      <TableCell className="px-4 py-4">
                        <Badge variant="outline" className="text-[9px] font-bold py-0.5 px-1.5 border-muted-foreground/25 text-muted-foreground bg-muted/20">
                          {tx.category}
                        </Badge>
                      </TableCell>
                      <TableCell className={`px-4 py-4 text-right font-bold tabular-nums text-sm ${tx.amount > 0 ? "text-emerald-500" : "text-foreground"}`}>
                        {tx.amount > 0 ? "+" : ""}${tx.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                      </TableCell>
                      <TableCell className="px-4 py-4">
                        <Badge className={`text-[10px] font-bold py-0 px-2 border-0 ${
                          tx.status === "Posted" ? "bg-emerald-500/10 text-emerald-500 dark:text-emerald-400" : "bg-amber-500/10 text-amber-500 dark:text-amber-400"
                        }`}>
                          {tx.status}
                        </Badge>
                      </TableCell>
