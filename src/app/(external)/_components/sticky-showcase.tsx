...
const features = [
  {
    ...
    description: "Monitor expenses, manage subscriptions, and get AI-powered financial insights. See your complete financial picture in one view.",
  ...
  },
  {
    ...
    description: "Kanban boards, smart reminders, and AI-prioritized task lists. Never let anything slip through the cracks.",
  ...
  },
  {
    ...
    description: "Zero-knowledge encryption, biometric auth, and secure document storage. We can't read your data — and neither can anyone else.",
      ...
      <div className="mt-4 flex flex-col gap-3">
        {tasks.map((t) => (
          <div key={t.text} className="flex items-center gap-3 text-sm">
            <div className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 ${t.done ? "bg-[#6366F1] border-[#6366F1]" : "border-white/20"}`}>
  ...
  return (
    <div className="bg-[#0A0A1A]/80 border border-white/[0.06] rounded-2xl p-6 backdrop-blur-sm">
      ...
      <div className="flex items-center gap-3 mt-4">
        ...
        <span className="text-xs font-medium text-emerald-400 bg-emerald-400/10 px-2.5 py-1 rounded-full">Protected</span>
      </div>
        ...
        <div className="flex flex-col">
          {features.map((feature, idx) => (
            ...
            >
              ...
              <h3 className={`text-2xl font-bold mt-2 transition-colors duration-500 ${activeIndex === idx ? "text-white" : "text-gray-700"}`}>
                {feature.title}
              </h3>
              <p className={`mt-4 leading-relaxed transition-colors duration-500 ${activeIndex === idx ? "text-gray-400" : "text-gray-700"}`}>
                {feature.description}
              </p>
              <div className="mt-5 flex flex-col gap-2.5">
                {feature.bullets.map((bullet) => (
                  ...
                  >
                    <Check className="size-4 flex-shrink-0" style={{ color: activeIndex === idx ? feature.labelColor : "rgba(255,255,255,0.15)" }} />
                    {bullet}
                  </div>
                ))}
              </div>
